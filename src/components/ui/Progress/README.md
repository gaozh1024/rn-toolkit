...
<Progress variant="linear" value={0.6} color="success" trackColor="divider" thickness={6} />
```

### 线性 - 不确定态
```tsx
<Progress variant="linear" indeterminate color="primary" thickness={6} />
```

### 环形 - 确定态（含中心文案）
```tsx
<Progress variant="circular" value={0.75} color="#7C3AED" trackColor="divider" size={56} thickness={6} showLabel />
```
