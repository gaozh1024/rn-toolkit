...
export function useClipboard(): UseClipboardReturn {
  ...
  const copyToClipboard = useCallback(async (text: string, options?: ClipboardOptions): Promise<boolean> => {
    setIsLoading(true);
    try {
      const success = await ClipboardService.copyToClipboard(text, options);
      if (success) setClipboardText(text);
      return success;
    } finally {
      setIsLoading(false);
    }
  ...
  const copyObject = useCallback(async (obj: any, options?: ClipboardOptions): Promise<boolean> => {
    setIsLoading(true);
    try {
